...
function TooltipCustom({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-lg p-3 shadow-sm text-sm">
      ...
      <p className="text-[#065F46]">Comissão: <span className="tabular-nums font-medium">{formatarMoeda(payload[0].value)}</span></p>
    </div>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <AreaChart data={dados} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <Area
          type="monotone" dataKey="comissao" stroke="#065F46" strokeWidth={2}
          fill="url(#gradComissao)" dot={{ fill: "#065F46", r: 3 }}
        />
      </AreaChart>
    </ResponsiveContainer>
  );
